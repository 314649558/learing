...
object ESUtils {
  ...
  def indexExists(client:TransportClient,indexName:String): Boolean ={
    val request: IndicesExistsRequest =new IndicesExistsRequest(indexName)
    val response: IndicesExistsResponse = client.admin().indices().exists(request).actionGet()
